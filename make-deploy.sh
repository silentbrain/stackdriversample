...
- name: 'gcr.io/cloud-builders/gcloud'
  args: ['compute', 'ssh', 'pping@${INSTANCE_NAME}', '--zone', '${ZONE}', '--tunnel-through-iap', '--ssh-flag', '-p 10022', '--command', 'gsutil -m cp -r \${_GIT_BUCKET} gs://kty-test1/after.sh /hosting/pping/ && sh /hosting/pping/after.sh']
